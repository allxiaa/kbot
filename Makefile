REGISTRY ?= ghcr.io
REPOSITORY ?= sae4xxx/$(shell basename $(shell git remote get-url origin))
...
image:
	docker --debug build . -t $(REGISTRY)/$(REPOSITORY):$(VERSION)-${TARGETOS}-$(TARGETARCH)
...
clean:
	rm -rf kbot
	docker rmi $(REGISTRY)/$(REPOSITORY):$(VERSION)-${TARGETOS}-$(TARGETARCH) || true